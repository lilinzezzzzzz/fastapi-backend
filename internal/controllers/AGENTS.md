...
- 响应统一使用 `BaseResponse[T]` / `BaseListResponse[T]` 信封在 Controller 层组装，Service 只返回业务数据，不关心 envelope。
...
- 调用一个明确的 Service 方法完成用例，Service 返回业务数据，Controller 负责包装成 `BaseResponse`。
- 直接返回 `BaseResponse` 实例或可被 response model 正确序列化的对象。
