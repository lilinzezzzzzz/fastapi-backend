...
class UserDao(BaseDao[User]):
    ...
    async def get_by_wechat_openid(self, openid: str) -> User | None:
        """通过微信 openid 查询用户"""
        return await self.querier.eq_(self.model_cls.wechat_openid, openid).first()

    async def is_wechat_openid_exist(self, openid: str) -> bool:
        """检查微信 openid 是否已存在"""
        user = await self.querier.eq_(self.model_cls.wechat_openid, openid).first()
        return user is not None

    async def get_by_alipay_user_id(self, user_id: str) -> User | None:
        """通过支付宝用户 ID 查询用户"""
        return await self.querier.eq_(self.model_cls.alipay_user_id, user_id).first()

    async def is_alipay_user_id_exist(self, user_id: str) -> bool:
        """检查支付宝用户 ID 是否已存在"""
        user = await self.querier.eq_(self.model_cls.alipay_user_id, user_id).first()
        return user is not None
