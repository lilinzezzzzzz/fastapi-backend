from internal.infra import redis
...
async def get_cache_user_info(token: str) -> dict | None:
    token_value = await redis.cache_client.get_value(token_cache_key(token))
...
async def verify_token(token: str) -> tuple[dict | None, bool]:
    ...
    # 检查有没有在token 列表里
    token_list = await redis.cache_client.get_list(token_list_cache_key(user_id))
