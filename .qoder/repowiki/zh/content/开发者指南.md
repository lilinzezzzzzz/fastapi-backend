...
- [docs/third_party_login_guide.md](file://docs/third_party_login_guide.md)
- [docs/uv_use_guide.md](file://docs/uv_use_guide.md)
</cite>

## 更新摘要
**所做更改**
- 移除了对已删除AGENTS.md文件的引用
- 更新了项目结构描述以反映实际的内部API路由组织
- 更新了代码规范和最佳实践部分
- 更新了文档规范以包含最新的文档指南
- 修正了控制器层的路由组织描述
...
CtrlPublic["internal/controllers/publicapi/"]
CtrlInternal["internal/controllers/internalapi/"]
CtrlService["internal/controllers/serviceapi/"]
end
subgraph "服务"
...
subgraph "基础设施"
InfraDB["internal/infra/database/connection.py"]
InfraRedis["internal/infra/redis/connection.py"]
InfraAnyIO["internal/utils/anyio_task.py"]
InfraCelery["internal/tasks/celery_tasks.py"]
InfraAPS["internal/utils/apscheduler/scheduler.py"]
end
subgraph "工具库"
...
- [configs/.secrets.example](file://configs/.secrets.example#L1-L20)
...
- [pyproject.toml](file://pyproject.toml#L1-L157)
- [main.py](file://main.py#L1-L4)
- [internal/app.py](file://internal/app.py#L1-L109)
...
- 任务注册：通过 internal/tasks/celery_tasks.py 注册任务，配合 APScheduler 与 AnyIO 任务管理。

```mermaid
flowchart TD
W["run_celery_worker.sh"] --> Args["解析命令行参数"]
...
**图表来源**
- [scripts/run_celery_worker.sh](file://scripts/run_celery_worker.sh#L6-L35)
...
- [scripts/run_celery_worker.sh](file://scripts/run_celery_worker.sh#L7-L10)
...
### 文档规范
- Markdown：使用 docs/auth_module_guide.md、docs/md_use_guide.md、docs/dataclass_use_guide.md、docs/third_party_login_guide.md、docs/uv_use_guide.md 等作为参考格式。
...
**章节来源**
- [pyproject.toml](file://pyproject.toml#L108-L157)
- [.pre-commit-config.yaml](file://.pre-commit-config.yaml#L1-L7)
