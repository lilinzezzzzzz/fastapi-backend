...
## 更新摘要
**变更内容**
- 所有核心功能已整合到 `pkg/crypter/__init__.py` 中，实现了统一的模块接口
- 新增 `register_algorithm` 装饰器和 `StrEnum` 枚举系统
- 改进了 `BaseCryptoUtil` 基类和工厂函数结构
- 内部模块重构，移除了独立的 `base.py` 文件
- 应用级懒加载机制保持不变，仍通过 `internal/utils/crypto.py` 实现
...
**更新** 加密工具包已完成模块重构，所有核心功能现已整合到单一的 `__init__.py` 文件中，提供了更加统一和可扩展的接口设计。

## 项目结构
加密工具包位于 `pkg/crypter`，经过重构后采用统一的模块接口设计：
- 统一接口：`pkg/crypter/__init__.py`（包含所有核心功能）
...
graph TB
subgraph "重构后的加密工具包"
Init["pkg/crypter/__init__.py<br/>统一接口与注册中心"]
AES["AESCipher<br/>Fernet 包装实现"]
Factory["get_crypto_class<br/>工厂函数"]
Register["register_algorithm<br/>装饰器注册"]
Enum["EncryptionAlgorithm<br/>StrEnum 枚举"]
end
subgraph "应用集成"
...
- **EncryptionAlgorithm**：算法枚举，使用 `StrEnum` 实现，当前包含 AES；预留扩展（如 SM4）。
...
**更新** 所有核心功能已整合到 `__init__.py`，提供了更加统一的接口访问方式。

**章节来源**
- [pkg/crypter/__init__.py](file://pkg/crypter/__init__.py#L25-L106)
...
- **设计要点**
  ...
  - 通过 `@register_algorithm` 装饰器自动注册到全局注册表。
- **错误处理**
...
### 注册与工厂系统
- **register_algorithm**：装饰器，将具体算法类注册到全局注册表 `_ALGORITHM_REGISTRY`，确保 `get_crypto_class` 可按枚举返回正确实现。
- **get_crypto_class**：根据算法枚举返回对应类，若未注册则抛出未实现错误，便于扩展新算法。
- **EncryptionAlgorithm**：使用 `StrEnum` 实现的算法枚举，提供类型安全的算法标识。
...
**图表来源**
- [pkg/crypter/__init__.py](file://pkg/crypter/__init__.py#L81-L95)
- [pkg/crypter/__init__.py](file://pkg/crypter/__init__.py#L34-L43)
...
**更新** 所有核心功能已整合到 `__init__.py`，简化了模块间的依赖关系。
...
- [pkg/crypter/__init__.py](file://pkg/crypter/__init__.py#L31-L95)
- [internal/utils/crypto.py](file://internal/utils/crypto.py#L7-L18)
...
- **安全**
  - 密钥必须是 32 字节 URL 安全 Base64 编码，且妥善保管；建议使用 `AESCipher.generate_key()` 生成并在安全渠道分发与存储。
  - 同一明文多次加密得到的密文不同（由随机 IV 与 HMAC 保证），避免统计分析。
  - 解密失败严格区分"令牌无效/密钥错误"与"篡改"，便于定位问题。
  - 环境变量中不应硬编码密钥，应通过安全配置源注入（参考环境示例文件）。

**更新** 模块重构后，接口更加统一，减少了模块间的耦合度，提高了代码的可维护性和扩展性。

**章节来源**
- [pkg/crypter/aes.py](file://pkg/crypter/aes.py#L26-L28)
- [pkg/crypter/aes.py](file://pkg/crypter/aes.py#L30-L43)
...
- **"密钥无效"或"空密钥"**
  ...
  - 处理：确认密钥为 32 字节 URL 安全 Base64 编码；可通过 `AESCipher.generate_key()` 生成新密钥并替换。
- **"解密失败：令牌无效或密钥错误"**
  - 现象：解密抛出错误。
...
- **"算法未注册"**
  - 现象：调用 `get_crypto_class()` 抛出 `NotImplementedError`。
  - 处理：确认算法已在 `__init__.py` 中注册，或检查导入顺序。
- **"测试用例覆盖点"**
...
- [tests/toolkit/test_crypto.py](file://tests/toolkit/test_crypto.py#L118-L183)
- [tests/toolkit/test_crypto.py](file://tests/toolkit/test_crypto.py#L196-L199)

## 结论
本加密工具包经过重构后，以统一的模块接口实现了对称加密，提供清晰的密钥管理、便捷的工厂与懒加载机制，以及完善的测试覆盖。新的 `__init__.py` 文件整合了所有核心功能，提供了更加统一和可扩展的接口设计。配合 bcrypt 的密码哈希方案，满足常见数据保护与身份验证需求。建议在生产环境中遵循密钥安全策略、合理选择 bcrypt 轮数，并在应用启动阶段完成初始化与密钥注入。

**更新** 模块重构显著提升了代码的组织性和可维护性，为未来的算法扩展奠定了良好的基础。
...
- **工厂与注册**
  ...
  - 算法枚举：[pkg/crypter/__init__.py](file://pkg/crypter/__init__.py#L25-L28)
  - 注册装饰器：[pkg/crypter/__init__.py](file://pkg/crypter/__init__.py#L34-L43)
- **应用初始化**
...
**更新** 新增了统一接口的使用示例，展示了重构后的模块组织方式。

### 安全最佳实践
- **密钥管理**
  - 使用 `AESCipher.generate_key()` 生成密钥；通过安全配置源注入；避免硬编码在代码或环境文件中。
- **输入校验**
...
- **模块使用最佳实践**
  - 优先使用统一接口：`from pkg.crypter import AESCipher, get_crypto_class, EncryptionAlgorithm`
  - 利用工厂函数获取实现类，便于未来扩展新算法。
...
- **扩展性**
  - 新增算法时，只需实现 `BaseCryptoUtil` 接口并通过 `@register_algorithm` 装饰器注册即可。
...
- 利用统一接口进行调试：`from pkg.crypter import *` 可直接访问所有核心组件。
- 检查算法注册状态：通过 `get_crypto_class(EncryptionAlgorithm.AES)` 验证注册是否正常。
