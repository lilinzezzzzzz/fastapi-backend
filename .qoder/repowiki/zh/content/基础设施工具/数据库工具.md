...
## 更新摘要
**变更内容**
- ModelMixin.save 方法重构：原 save 方法重命名为 insert，新增统一 save 方法实现插入/更新一体化
- update 方法改为同步操作：从异步方法变为同步方法
- 新增 QueryBuilder.values() 和 exists() 方法
- DAO 层新增 col_querier() 和 col_counter() 方法
- CountBuilder 增强支持自定义 SQL 语句
...
- [pkg/database/dao.py](file://pkg/database/dao.py#L1-L320)
...
**章节来源**
- [pkg/database/base.py](file://pkg/database/base.py#L60-L409)
- [pkg/database/builder.py](file://pkg/database/builder.py#L18-L335)
- [pkg/database/dao.py](file://pkg/database/dao.py#L15-L320)
...
- 单对象操作：**重构** insert（严格插入）、**更新改为同步** update（同步更新）、save（统一插入/更新一体化）、soft_delete/restore（软删除）。
...
**更新** ModelMixin.save 方法重构：原 save 方法重命名为 insert，新增统一 save 方法实现插入/更新一体化
...
class ModelMixin {
+create(**kwargs) Self
+insert(session_provider, execute) Insert|None
+update(**kwargs) Self
...
- 执行：all()/first()，**新增** values()（返回列查询的元组列表）、exists()（检查记录是否存在）。
- 异常包装与提交。

**更新** 新增 QueryBuilder.values() 和 exists() 方法
...
Page --> |否| ValuesOrExists{"是否 values()/exists()?"}
Paginate --> ValuesOrExists
ValuesOrExists --> |values()| ValuesExec["执行 values()<br/>返回元组列表"]
ValuesOrExists --> |exists()| ExistsExec["执行 first()<br/>返回布尔值"]
ValuesOrExists --> |all()/first()| Exec["执行 all()/first()"]
ValuesExec --> Commit["提交事务"]
ExistsExec --> Commit
Exec --> Commit
Commit --> End(["结束"])
```
...
**图表来源**
- [pkg/database/builder.py](file://pkg/database/builder.py#L105-L208)
...
**图表来源**
- [pkg/database/builder.py](file://pkg/database/builder.py#L247-L335)
- [pkg/database/dao.py](file://pkg/database/dao.py#L176-L183)
...
- **增强** 支持自定义 SQL 语句：通过 custom_stmt 参数传入自定义 Select 语句。

**更新** CountBuilder 增强支持自定义 SQL 语句
...
**更新** DAO 层新增 col_querier() 和 col_counter() 方法
...
+query_by_primary_id(...)
+query_by_ids(...)
+col_querier(columns, include_deleted) QueryBuilder
+col_counter(count_column, is_distinct) CountBuilder
}
class UserDao {
+get_by_phone(phone)
...
## 故障排查指南
- 严格模式错误
  - insert/update 严格区分插入/更新：对持久化对象调用 insert 会抛错；对新对象调用 update 会抛错。
...
- [pkg/database/dao.py](file://pkg/database/dao.py#L223-L320)
...
- 事务执行器
  - 示例路径：[pkg/database/dao.py](file://pkg/database/dao.py#L223-L320)
...
#### ModelMixin.save 方法使用
**更新** ModelMixin.save 方法重构：原 save 方法重命名为 insert，新增统一 save 方法实现插入/更新一体化

```python
# 新对象插入
user = User.create(username="alice")
await user.save(db_session)  # 自动插入

# 已存在对象更新
db_user = await user_dao.query_by_primary_id(user.id)
db_user.update(username="alice_updated")
await db_user.save(db_session)  # 自动更新
```

#### QueryBuilder.values() 和 exists() 使用
**更新** 新增 QueryBuilder.values() 和 exists() 方法
...
#### DAO 层 col_querier() 和 col_counter() 使用
**更新** DAO 层新增 col_querier() 和 col_counter() 方法
...
#### CountBuilder 自定义 SQL 使用
**更新** CountBuilder 增强支持自定义 SQL 语句

```python
# 使用自定义 SQL 语句
custom_stmt = select(func.count(distinct(Model.dept_id))).select_from(Model)
custom_count = CountBuilder(
    Model,
    session_provider=db_session,
    custom_stmt=custom_stmt
)
result = await custom_count.count()
```