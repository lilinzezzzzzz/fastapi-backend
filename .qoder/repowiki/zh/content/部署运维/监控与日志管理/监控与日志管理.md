...
- [internal/__init__.py](file://internal/__init__.py)
...
**更新** 本版本增加了对BASE_LOG_DIR常量和改进的日志初始化API的支持说明。

## 项目结构
围绕监控与日志的关键文件分布如下：
- 应用入口与生命周期：main.py、internal/app.py
- 日志目录配置：internal/__init__.py（新增BASE_LOG_DIR常量）
...
- 配置模型与日志格式：internal/config/settings.py
...
G["internal/__init__.py<br/>BASE_LOG_DIR常量"] --> B
H["internal/config/settings.py<br/>日志格式配置"] --> D
I["pkg/toolkit/context.py<br/>请求上下文/追踪ID"] --> C
J["pkg/toolkit/response.py<br/>统一错误响应"] --> C
K["internal/core/exception.py<br/>业务异常模型"] --> C
L["pkg/toolkit/logger.py<br/>兼容性导出"] --> D
M["tests/logger/*.py<br/>日志与轮转测试"] --> D
N["configs/.env.*<br/>运行环境"] --> B
```

**图表来源**
- [main.py](file://main.py#L1-L18)
- [internal/app.py](file://internal/app.py#L1-L111)
- [internal/__init__.py](file://internal/__init__.py#L1-L5)
...
- [tests/logger/test_logger_rotation.py](file://tests/logger/test_logger_rotation.py#L1-L228)
...
- 日志目录配置中心
  - 通过BASE_LOG_DIR常量实现集中式日志目录配置，支持自定义日志根目录路径。
- 访问与异常记录中间件
...
**更新** 新增日志目录配置中心组件，通过BASE_LOG_DIR常量实现集中式日志目录管理。
...
BASEDIR["internal/__init__.py<br/>BASE_LOG_DIR常量"]
SETT["internal/config/settings.py<br/>日志格式配置"]
...
**图表来源**
- [pkg/logger/handler.py](file://pkg/logger/handler.py#L21-L461)
- [pkg/logger/__init__.py](file://pkg/logger/__init__.py#L45-L118)
...
### 日志目录配置中心
- BASE_LOG_DIR常量
  - 通过internal/__init__.py中的BASE_LOG_DIR常量实现集中式日志目录配置，默认指向项目根目录下的logs文件夹。
  - 支持自定义日志根目录路径，便于容器化部署和多环境配置。
- 目录管理策略
  - 支持use_subdir参数控制日志目录结构：True时按命名空间创建子目录，False时所有日志存放在base_log_dir根目录。
  - 提供目录创建与降级策略，确保日志系统在各种环境下稳定运行。
...
**章节来源**
- [pkg/logger/handler.py](file://pkg/logger/handler.py#L220-L461)
- [tests/logger/test_logger.py](file://tests/logger/test_logger.py#L69-L149)
- [tests/logger/test_logger_rotation.py](file://tests/logger/test_logger_rotation.py#L10-L228)
...
- [internal/app.py](file://internal/app.py#L85-L111)
...
**章节来源**
- [pkg/logger/handler.py](file://pkg/logger/handler.py#L146-L461)
...
**章节来源**
- [pkg/logger/handler.py](file://pkg/logger/handler.py#L270-L332)
- [internal/middlewares/recorder.py](file://internal/middlewares/recorder.py#L92-L111)
...
- [pkg/logger/handler.py](file://pkg/logger/handler.py#L146-L332)
...
BASEDIR["BASE_LOG_DIR"] --> APP["应用"]
SETT["日志格式配置"] --> INIT["初始化函数"]
```
...
## 结论
本项目通过统一的日志入口与中间件实现了完善的访问与异常记录，结合灵活的轮转与保留策略，满足生产环境的可观测性需求。新增的BASE_LOG_DIR常量提供了集中式日志目录配置能力，改进的日志初始化API提升了API可用性和灵活性。建议在此基础上进一步对接日志与指标平台，完善实时监控与告警体系，并持续优化日志格式与检索策略以提升故障诊断效率。

**更新** 本版本增强了日志目录管理能力和API可用性，为生产环境部署提供了更好的支持。
