...
## 更新摘要
**变更内容**
- 更新Redis DAO文件位置：从internal/dao/redis.py迁移到internal/cache/redis.py
- 更新所有导入路径：从internal.dao.redis更新为internal.cache.redis
- 更新文件结构引用：反映新的目录组织方式
- 更新使用示例和代码路径引用
...
**更新** 文件位置已从internal/dao/redis.py迁移到internal/cache/redis.py，反映了更加清晰的目录结构组织。
...
- [internal/controllers/api/auth.py](file://internal/controllers/api/auth.py#L1-L143)
...
**章节来源**
- [internal/cache/redis.py](file://internal/cache/redis.py#L6-L37)
...
Controller->>Dao : set_dict(令牌键, 用户元数据, 过期时间)
Dao->>Cache : set_dict(生成的令牌键, 用户元数据)
Cache->>Redis : SET token : {token} 用户元数据 EX 1800
Redis-->>Cache : OK
Cache-->>Dao : 操作成功
Dao-->>Controller : 返回成功
Controller->>Dao : push_to_list(用户令牌列表键, token)
Dao->>Cache : push_to_list(生成的令牌列表键, token)
...
**图表来源**
- [internal/controllers/api/auth.py](file://internal/controllers/api/auth.py#L63-L69)
...
| `remove_from_list` | 从列表移除元素 | key, value | int |
...
**更新** 新的文件位置（internal/cache/redis.py）反映了更加清晰的目录结构，将缓存相关的DAO专门放置在cache目录下，提高了代码组织的逻辑性。
...
- **创建CacheDao实例**：[internal/cache/redis.py](file://internal/cache/redis.py#L40-L41)
- **获取用户元数据**：[internal/cache/redis.py](file://internal/cache/redis.py#L19-L25)
- **获取用户令牌列表**：[internal/cache/redis.py](file://internal/cache/redis.py#L27-L33)
- **从列表移除元素**：[internal/cache/redis.py](file://internal/cache/redis.py#L35-L37)
- **认证逻辑使用**：[internal/core/auth.py](file://internal/core/auth.py#L1-L24)
- **控制器使用**：[internal/controllers/api/auth.py](file://internal/controllers/api/auth.py#L8-L8)
