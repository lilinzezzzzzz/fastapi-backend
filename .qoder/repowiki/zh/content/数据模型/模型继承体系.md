...
## 更新摘要
**变更内容**
- 更新了BaseDao类的实例操作代理方法部分，新增了四个代理方法的详细说明
- 增强了DAO层功能性和易用性的文档描述
- 更新了使用示例，展示新的代理方法的使用方式
...
- BaseDao[T]：DAO 抽象基类，提供查询构建器、计数器、更新器及常用查询方法，**新增实例操作代理方法**。
...
## 架构总览
ModelMixin 通过统一的字段定义与方法，为所有业务模型提供一致的生命周期管理与操作接口。DAO 层通过泛型约束确保类型安全，查询/更新构建器提供链式条件构造与自动软删除过滤。会话提供者贯穿整个流程，保证事务一致性与资源管理。**新增的实例操作代理方法进一步简化了DAO层的使用，使开发者可以直接通过DAO实例进行模型操作**。
...
- **实例操作代理方法**：**新增的 insert、save、soft_delete、restore 方法，直接代理 ModelMixin 实例操作，自动传入 session_provider 参数**。
...
+query_by_ids(ids) T[]
+insert(instance : T) None
+save(instance : T) T|None
+soft_delete(instance : T) None
+restore(instance : T) None
}
...
### 实例操作代理方法详解
**新增** BaseDao 类现在包含四个实例操作代理方法，显著增强了DAO层的功能性和易用性：

- **insert(instance)**：插入新实例，代理 ModelMixin 实例的 insert() 方法，自动传入 session_provider 参数
- **save(instance)**：保存实例，代理 ModelMixin 实例的 save() 方法，自动传入 session_provider 参数
- **soft_delete(instance)**：软删除实例，代理 ModelMixin 实例的 soft_delete() 方法，自动传入 session_provider 参数
- **restore(instance)**：恢复已删除的实例，代理 ModelMixin 实例的 restore() 方法，自动传入 session_provider 参数

这些代理方法的优势：
- **简化API**：开发者可以直接通过DAO实例进行模型操作，无需手动传入 session_provider
- **类型安全**：保持与 ModelMixin 方法相同的签名和行为
- **一致性**：与 DAO 其他方法保持一致的使用模式

**章节来源**
- [pkg/database/dao.py](file://pkg/database/dao.py#L197-L221)
...
- **实例操作代理方法通过自动传入 session_provider，进一步简化了DAO层的使用**。
...
- **实例操作代理方法**：**通过减少手动传参的开销，进一步提升了DAO层的使用效率**。
...
- **实例操作代理方法**：**使用代理方法时，无需手动传入 session_provider 参数，避免了常见的参数传递错误**。
...
## 结论
ModelMixin 通过统一的字段、方法与工具，为所有业务模型提供了标准化的生命周期管理与操作接口。结合 DAO 与查询/更新构建器，实现了类型安全、事务一致与性能优化的模型层架构。**新增的实例操作代理方法进一步简化了DAO层的使用，使开发者能够更专注于业务逻辑而非底层细节**。遵循本文的最佳实践与安全考虑，可确保新模型的正确继承与高效维护。
...
- **实例操作代理方法**：**优先使用DAO的代理方法进行模型操作，享受自动参数传递和类型安全的优势**。
...
- **实例操作代理方法**：**使用 userDao.insert(instance)、userDao.save(instance)、userDao.soft_delete(instance)、userDao.restore(instance) 等代理方法进行模型操作**。
...
- **实例操作代理方法**：**代理方法自动处理 session_provider 参数，减少了手动传参可能引入的安全风险**。
