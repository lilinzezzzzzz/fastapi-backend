# 用户模型

<cite>
**本文引用的文件**
- [internal/models/user.py](file://internal/models/user.py)
- [internal/services/user.py](file://internal/services/user.py)
- [internal/utils/password.py](file://internal/utils/password.py)
- [pkg/database/base.py](file://pkg/database/base.py)
- [pkg/database/dao.py](file://pkg/database/dao.py)
- [pkg/database/builder.py](file://pkg/database/builder.py)
- [internal/dao/user.py](file://internal/dao/user.py)
- [internal/dtos/user.py](file://internal/dtos/user.py)
- [internal/schemas/user.py](file://internal/schemas/user.py)
- [internal/infra/database.py](file://internal/infra/database.py)
- [internal/controllers/api/user.py](file://internal/controllers/api/user.py)
- [tests/orm/test_orm.py](file://tests/orm/test_orm.py)
</cite>

## 更新摘要
**变更内容**
- 新增 password_hash 字段支持密码哈希存储
- 增强用户认证功能，支持密码加密与验证
- 新增 PasswordHandler 工具类提供密码处理能力
- 更新服务层 UserService 以支持密码相关操作
- 完善用户模型的认证安全机制

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件系统性地文档化用户模型的设计与实现，覆盖字段定义、ORM 映射、数据库表结构、验证规则、业务约束与数据完整性要求。重点介绍新增的密码哈希存储功能，包括 bcrypt 加密算法的应用、密码验证机制以及完整的认证流程。同时说明用户模型与 DAO 层、服务层、API 控制器之间的交互方式。

## 项目结构
用户模型位于 internal/models/user.py，采用 SQLAlchemy 2.x 声明式映射，继承通用 ModelMixin 提供统一的 CRUD、软删除、上下文字段填充等能力。新增的密码哈希功能通过 PasswordHandler 工具类实现，服务层 UserService 提供密码验证和用户创建功能。DAO 层基于 pkg/database/dao.py 的 BaseDao 封装查询构建器，API 控制器通过依赖注入使用服务层。

```mermaid
graph TB
subgraph "模型层"
M["User 模型<br/>internal/models/user.py"]
PH["PasswordHandler 工具类<br/>internal/utils/password.py"]
end
subgraph "基础设施"
DB["数据库引擎/会话<br/>internal/infra/database.py"]
end
subgraph "DAO 层"
D["UserDao<br/>internal/dao/user.py"]
BD["BaseDao<br/>pkg/database/dao.py"]
QB["QueryBuilder/CountBuilder<br/>pkg/database/builder.py"]
end
subgraph "服务层"
S["UserService<br/>internal/services/user.py"]
end
subgraph "API 控制器"
C1["API 控制器<br/>internal/controllers/api/user.py"]
end
M --> D
D --> BD
BD --> QB
S --> D
S --> PH
C1 --> S
DB --> D
```

**图表来源**
- [internal/models/user.py](file://internal/models/user.py#L1-L14)
- [internal/utils/password.py](file://internal/utils/password.py#L1-L84)
- [internal/dao/user.py](file://internal/dao/user.py#L1-L31)
- [pkg/database/dao.py](file://pkg/database/dao.py#L1-L203)
- [pkg/database/builder.py](file://pkg/database/builder.py#L1-L200)
- [internal/services/user.py](file://internal/services/user.py#L1-L77)
- [internal/infra/database.py](file://internal/infra/database.py#L1-L154)
- [internal/controllers/api/user.py](file://internal/controllers/api/user.py#L1-L17)

**章节来源**
- [internal/models/user.py](file://internal/models/user.py#L1-L14)
- [internal/dao/user.py](file://internal/dao/user.py#L1-L31)
- [pkg/database/dao.py](file://pkg/database/dao.py#L1-L203)
- [pkg/database/builder.py](file://pkg/database/builder.py#L1-L200)
- [internal/services/user.py](file://internal/services/user.py#L1-L77)
- [internal/infra/database.py](file://internal/infra/database.py#L1-L154)
- [internal/controllers/api/user.py](file://internal/controllers/api/user.py#L1-L17)

## 核心组件
- 用户模型 User：定义 username、account、phone、password_hash 四个字段，长度分别为 64、64、11、255；继承 ModelMixin，具备统一的创建、更新、软删除、上下文字段填充等能力。
- 密码处理器 PasswordHandler：提供密码加密、验证和重哈希检查功能，基于 bcrypt 算法实现安全的密码存储。
- DAO 层 UserDao：基于 BaseDao 封装查询构建器，提供按手机号查询与存在性检查。
- 服务层 UserService：对外暴露业务方法，包含密码验证、用户创建、按手机号查询等功能。
- DTO/Schemas：UserListDto 与 UserDetailSchema 用于响应序列化与传输。
- 基础设施：数据库引擎与会话管理，提供 SessionProvider。

**章节来源**
- [internal/models/user.py](file://internal/models/user.py#L1-L14)
- [internal/utils/password.py](file://internal/utils/password.py#L1-L84)
- [pkg/database/base.py](file://pkg/database/base.py#L60-L364)
- [internal/dao/user.py](file://internal/dao/user.py#L1-L31)
- [pkg/database/dao.py](file://pkg/database/dao.py#L1-L203)
- [internal/dtos/user.py](file://internal/dtos/user.py#L1-L20)
- [internal/schemas/user.py](file://internal/schemas/user.py#L1-L43)
- [internal/infra/database.py](file://internal/infra/database.py#L1-L154)

## 架构总览
用户模型贯穿"控制器 → 服务层 → DAO 层 → 数据库"的调用链路，使用 SessionProvider 管理会话生命周期，确保事务一致性与资源释放。新增的密码哈希功能通过 PasswordHandler 工具类提供安全的密码处理能力。

```mermaid
sequenceDiagram
participant Client as "客户端"
participant Ctrl as "API 控制器"
participant Svc as "UserService"
participant PH as "PasswordHandler"
participant Dao as "UserDao"
participant Q as "QueryBuilder"
participant DB as "数据库"
Client->>Ctrl : 发起用户注册/登录请求
Ctrl->>Svc : 调用业务方法
alt 用户注册
Svc->>PH : hash_password(原始密码)
PH-->>Svc : 返回密码哈希
Svc->>Dao : 创建用户(含密码哈希)
else 用户登录
Svc->>Dao : 查询用户
Dao-->>Svc : 返回用户信息
Svc->>PH : verify_password(输入密码, 存储哈希)
PH-->>Svc : 返回验证结果
end
Q->>DB : 执行查询/插入/更新
DB-->>Q : 返回结果
Q-->>Dao : 实体/None
Dao-->>Svc : 实体/None
Svc-->>Ctrl : 返回业务结果
Ctrl-->>Client : 响应
```

**图表来源**
- [internal/controllers/api/user.py](file://internal/controllers/api/user.py#L1-L17)
- [internal/services/user.py](file://internal/services/user.py#L22-L66)
- [internal/utils/password.py](file://internal/utils/password.py#L16-L83)
- [internal/dao/user.py](file://internal/dao/user.py#L9-L21)
- [pkg/database/builder.py](file://pkg/database/builder.py#L111-L163)
- [internal/infra/database.py](file://internal/infra/database.py#L85-L111)

## 详细组件分析

### 用户模型 User
- 字段定义与长度限制
  - username: 字符串，最大长度 64
  - account: 字符串，最大长度 64
  - phone: 字符串，最大长度 11
  - password_hash: 字符串，最大长度 255，支持 None 值
- ORM 映射与表结构
  - 继承 ModelMixin，自动获得 id、creator_id、created_at、updater_id、updated_at、deleted_at 等通用字段
  - 表名为 user
- 数据完整性与默认值
  - ModelMixin 在创建时填充 id（雪花算法）、created_at、updated_at、creator_id（来自上下文），更新时自动填充 updated_at 与 updater_id
- CRUD 与软删除
  - 支持 save（严格插入）、update（严格更新）、soft_delete（软删除）、restore（恢复）

```mermaid
classDiagram
class ModelMixin {
+BigInteger id
+BigInteger creator_id
+DateTime created_at
+BigInteger updater_id
+DateTime updated_at
+DateTime deleted_at
+create(**kwargs) Self
+save(session_provider, execute) Insert|None
+update(session_provider, **kwargs) Self
+soft_delete(session_provider) void
+restore(session_provider) void
}
class User {
+String username
+String account
+String phone
+String password_hash
}
User --|> ModelMixin : "继承"
```

**图表来源**
- [internal/models/user.py](file://internal/models/user.py#L7-L13)
- [pkg/database/base.py](file://pkg/database/base.py#L60-L364)

**章节来源**
- [internal/models/user.py](file://internal/models/user.py#L7-L13)
- [pkg/database/base.py](file://pkg/database/base.py#L60-L364)

### 密码处理器 PasswordHandler
- 功能特性
  - 使用 bcrypt 算法进行密码哈希，具有自动加盐、高计算成本、不可逆等安全特性
  - 支持密码加密、验证和重哈希检查
- 核心方法
  - hash_password(password: str) → str：对密码进行加密，返回哈希字符串
  - verify_password(password: str, password_hash: str) → bool：验证密码是否正确
  - needs_rehash(password_hash: str) → bool：检查密码哈希是否需要重新加密
- 安全考虑
  - rounds 参数控制计算成本，默认为 12
  - 自动处理异常情况，防止安全漏洞
  - 支持 bcrypt 算法升级检测

```mermaid
flowchart TD
Start(["密码处理请求"]) --> Type{"操作类型"}
Type --> |加密| Hash["hash_password<br/>bcrypt.gensalt + hashpw"]
Type --> |验证| Verify["verify_password<br/>checkpw + 异常处理"]
Type --> |检查| Rehash["needs_rehash<br/>hashpw_needs_rehash"]
Hash --> Salt["生成随机盐值"]
Salt --> Encrypt["加密密码"]
Encrypt --> Result1["返回哈希字符串"]
Verify --> Check["验证密码与哈希匹配"]
Check --> Valid{"验证结果"}
Valid --> |成功| TrueResult["返回 True"]
Valid --> |失败| FalseResult["返回 False"]
Rehash --> Need{"是否需要重哈希"}
Need --> |是| TrueResult2["返回 True"]
Need --> |否| FalseResult2["返回 False"]
Result1 --> End(["结束"])
TrueResult --> End
FalseResult --> End
TrueResult2 --> End
FalseResult2 --> End
```

**图表来源**
- [internal/utils/password.py](file://internal/utils/password.py#L16-L83)

**章节来源**
- [internal/utils/password.py](file://internal/utils/password.py#L1-L84)

### DAO 层：UserDao
- 查询构建器
  - querier：基于 QueryBuilder，按 updated_at 降序排列，排除软删除记录
  - counter：基于 CountBuilder，统计非软删除记录
- 关键方法
  - get_by_phone(phone: str) → User | None：按手机号精确查询第一条
  - get_by_username(username: str) → User | None：按用户名精确查询第一条
  - is_phone_exist(phone: str) → bool：统计是否存在该手机号

```mermaid
flowchart TD
Start(["进入 UserDao.get_by_phone"]) --> Build["构建查询条件: phone = ?"]
Build --> Exec["执行查询: first()"]
Exec --> Found{"是否找到记录？"}
Found --> |是| ReturnOne["返回 User 实例"]
Found --> |否| ReturnNone["返回 None"]
ReturnOne --> End(["结束"])
ReturnNone --> End
```

**图表来源**
- [internal/dao/user.py](file://internal/dao/user.py#L9-L16)
- [pkg/database/dao.py](file://pkg/database/dao.py#L47-L50)
- [pkg/database/builder.py](file://pkg/database/builder.py#L111-L163)

**章节来源**
- [internal/dao/user.py](file://internal/dao/user.py#L1-L31)
- [pkg/database/dao.py](file://pkg/database/dao.py#L1-L203)
- [pkg/database/builder.py](file://pkg/database/builder.py#L1-L200)

### 服务层：UserService
- 依赖注入
  - 通过 new_user_service() 提供单例 UserService，依赖 UserDao
- 核心方法
  - get_user_by_phone(phone: str) → User：委托 DAO 查询
  - get_user_by_username(username: str) → User：按用户名查询
  - verify_password(user: User, password: str) → bool：验证用户密码
  - create_user(username: str, account: str, phone: str, password: str) → User：创建新用户并加密密码

```mermaid
sequenceDiagram
participant Ctrl as "控制器"
participant Svc as "UserService"
participant PH as "PasswordHandler"
participant Dao as "UserDao"
Ctrl->>Svc : create_user(username, account, phone, password)
Svc->>Dao : is_phone_exist(phone)
Dao-->>Svc : False (手机号不存在)
Svc->>PH : hash_password(password)
PH-->>Svc : password_hash
Svc->>Dao : create(username, account, phone, password_hash)
Dao-->>Svc : User 实例
Svc-->>Ctrl : User 实例
Ctrl->>Svc : verify_password(user, password)
Svc->>Dao : get_user_by_phone(phone)
Dao-->>Svc : User 实例
Svc->>PH : verify_password(password, user.password_hash)
PH-->>Svc : True/False
Svc-->>Ctrl : True/False
```

**图表来源**
- [internal/services/user.py](file://internal/services/user.py#L14-L66)
- [internal/utils/password.py](file://internal/utils/password.py#L41-L68)
- [internal/dao/user.py](file://internal/dao/user.py#L9-L21)

**章节来源**
- [internal/services/user.py](file://internal/services/user.py#L1-L77)
- [internal/dao/user.py](file://internal/dao/user.py#L1-L31)

### DTO 与 Schema
- UserListDto：包含 total 与 items（列表元素为 User 实例），提供 to_response_schema() 与 to_dict() 方法
- UserDetailSchema：用于响应序列化，包含 id、name、phone
- 转换逻辑：to_response_schema() 将 User 列表转换为 UserDetailSchema 列表
- 登录/注册 Schema：UserLoginReqSchema 和 UserRegisterReqSchema 提供请求验证

```mermaid
classDiagram
class UserListDto {
+int total
+User[] items
+to_response_schema() UserListResponseSchema
+to_dict() dict
}
class UserDetailSchema {
+int id
+string name
+string phone
}
class UserLoginReqSchema {
+string username
+string password
}
class UserRegisterReqSchema {
+string username
+string password
+string phone
}
UserListDto --> UserDetailSchema : "转换 items"
UserLoginReqSchema --> UserDetailSchema : "登录响应"
UserRegisterReqSchema --> UserDetailSchema : "注册响应"
```

**图表来源**
- [internal/dtos/user.py](file://internal/dtos/user.py#L7-L19)
- [internal/schemas/user.py](file://internal/schemas/user.py#L8-L43)

**章节来源**
- [internal/dtos/user.py](file://internal/dtos/user.py#L1-L20)
- [internal/schemas/user.py](file://internal/schemas/user.py#L1-L43)

### API 控制器
- API 控制器：通过 Depends(new_user_service) 注入 UserService，提供基础的用户接口
- 支持依赖注入模式，便于测试和扩展

**章节来源**
- [internal/controllers/api/user.py](file://internal/controllers/api/user.py#L1-L17)

### 数据库会话与连接
- init_async_db()/close_async_db()：初始化/关闭数据库引擎与会话工厂
- get_session()：异步上下文管理器，提供会话并处理异常回滚
- BaseDao/ModelMixin：通过 SessionProvider 统一执行 SQL

**章节来源**
- [internal/infra/database.py](file://internal/infra/database.py#L26-L111)
- [pkg/database/dao.py](file://pkg/database/dao.py#L106-L203)
- [pkg/database/base.py](file://pkg/database/base.py#L283-L304)

## 依赖分析
- 模型与 DAO：UserDao._model_cls 指向 User，BaseDao 提供查询/计数/更新构建器
- 服务与 DAO：UserService 依赖 UserDao
- 服务与工具：UserService 依赖 PasswordHandler 进行密码处理
- 控制器与服务：控制器通过依赖注入获取 UserService
- 数据流：控制器 → 服务 → DAO → 查询构建器 → 数据库

```mermaid
graph LR
U["User 模型"] --> UD["UserDao"]
UD --> BD["BaseDao"]
BD --> QB["QueryBuilder/CountBuilder"]
S["UserService"] --> UD
S --> PH["PasswordHandler"]
C1["API 控制器"] --> S
DB["数据库"] <- --> QB
```

**图表来源**
- [internal/models/user.py](file://internal/models/user.py#L7-L13)
- [internal/dao/user.py](file://internal/dao/user.py#L6-L7)
- [pkg/database/dao.py](file://pkg/database/dao.py#L15-L42)
- [pkg/database/builder.py](file://pkg/database/builder.py#L111-L190)
- [internal/services/user.py](file://internal/services/user.py#L6-L8)
- [internal/utils/password.py](file://internal/utils/password.py#L6-L14)
- [internal/controllers/api/user.py](file://internal/controllers/api/user.py#L10)

**章节来源**
- [internal/models/user.py](file://internal/models/user.py#L1-L14)
- [internal/dao/user.py](file://internal/dao/user.py#L1-L31)
- [pkg/database/dao.py](file://pkg/database/dao.py#L1-L203)
- [pkg/database/builder.py](file://pkg/database/builder.py#L1-L200)
- [internal/services/user.py](file://internal/services/user.py#L1-L77)
- [internal/utils/password.py](file://internal/utils/password.py#L1-L84)
- [internal/controllers/api/user.py](file://internal/controllers/api/user.py#L1-L17)

## 性能考虑
- 查询构建器：QueryBuilder/CountBuilder 提供链式条件构造与分页，避免一次性加载大量数据
- 事务执行：execute_transaction 支持复杂业务在一个事务内完成，减少往返与锁竞争
- 会话管理：get_session 自动回滚异常，降低资源泄漏风险
- 密码处理：bcrypt 算法具有高计算成本，建议合理设置 rounds 参数平衡安全性与性能
- 建议
  - 对高频查询建立合适索引（如 phone）
  - 使用分页参数控制单次查询规模
  - 批量插入使用 ModelMixin.insert_rows/insert_instances
  - 为密码哈希字段建立适当索引以支持快速查找

**章节来源**
- [pkg/database/builder.py](file://pkg/database/builder.py#L111-L163)
- [pkg/database/dao.py](file://pkg/database/dao.py#L106-L203)
- [internal/infra/database.py](file://internal/infra/database.py#L85-L111)
- [internal/utils/password.py](file://internal/utils/password.py#L36-L37)

## 故障排查指南
- 插入/更新异常
  - save() 仅用于新对象插入，若对象已存在会抛出异常；update() 仅用于已存在记录更新
- 会话未初始化
  - get_session() 在未调用 init_async_db() 时会抛出异常
- 查询无结果
  - get_by_phone() 返回 None 表示不存在；is_phone_exist() 返回 False
- 事务失败
  - execute_transaction() 会在回调异常时自动回滚并抛出错误
- 密码处理异常
  - hash_password() 对空密码会抛出 ValueError
  - verify_password() 对无效哈希格式返回 False
  - needs_rehash() 可能因算法升级而返回 True

**章节来源**
- [pkg/database/base.py](file://pkg/database/base.py#L156-L200)
- [internal/infra/database.py](file://internal/infra/database.py#L85-L111)
- [pkg/database/dao.py](file://pkg/database/dao.py#L106-L203)
- [internal/utils/password.py](file://internal/utils/password.py#L20-L28)
- [internal/utils/password.py](file://internal/utils/password.py#L64-L68)

## 结论
用户模型采用清晰的分层架构：模型层提供字段与通用能力，DAO 层封装查询构建器，服务层编排业务，控制器负责接口接入。新增的密码哈希功能通过 PasswordHandler 工具类实现了安全的密码存储与验证机制。通过 SessionProvider 统一会话生命周期，结合查询构建器与事务执行器，保证了数据一致性与可维护性。建议在生产环境中补充字段级校验、索引优化、密码策略配置与单元测试覆盖。

## 附录

### 字段定义与约束
- username: 字符串，最大长度 64
- account: 字符串，最大长度 64
- phone: 字符串，最大长度 11
- password_hash: 字符串，最大长度 255，支持 None 值
- 通用字段（由 ModelMixin 提供）
  - id: BigInteger 主键（雪花算法）
  - creator_id: BigInteger 创建人
  - created_at: DateTime 创建时间
  - updater_id: BigInteger 更新人
  - updated_at: DateTime 更新时间
  - deleted_at: DateTime 软删除时间

**章节来源**
- [internal/models/user.py](file://internal/models/user.py#L10-L13)
- [pkg/database/base.py](file://pkg/database/base.py#L68-L74)

### ORM 映射与数据库表结构
- 表名：user
- 字段：id、username、account、phone、password_hash、creator_id、created_at、updater_id、updated_at、deleted_at
- 约束：id 为主键；建议为 phone 建立唯一索引以满足业务唯一性需求

**章节来源**
- [internal/models/user.py](file://internal/models/user.py#L8-L13)
- [pkg/database/base.py](file://pkg/database/base.py#L68-L74)

### 验证规则与业务约束
- 字段长度：username、account 最大 64，phone 最大 11，password_hash 最大 255
- 唯一性：建议在数据库层面为 phone 建唯一索引
- 软删除：deleted_at 非空表示记录软删除
- 上下文字段：creator_id/updater_id 来源于上下文，默认填充
- 密码安全：password_hash 字段存储 bcrypt 哈希值，支持空值表示未设置密码

**章节来源**
- [internal/models/user.py](file://internal/models/user.py#L10-L13)
- [pkg/database/base.py](file://pkg/database/base.py#L222-L251)

### CRUD 操作实现要点
- 创建
  - 使用 ModelMixin.create(...) 填充默认字段，随后 save(session_provider)
  - 用户创建时通过 PasswordHandler.hash_password() 加密密码
- 更新
  - 使用 update(session_provider, ...) 更新指定字段
  - 支持单独更新密码字段
- 查询
  - UserDao.get_by_phone(phone) 返回单条记录
  - UserDao.get_by_username(username) 返回单条记录
  - UserDao.is_phone_exist(phone) 返回布尔值
- 删除
  - 使用 soft_delete(session_provider) 进行软删除
  - 使用 restore(session_provider) 恢复软删除记录

**章节来源**
- [pkg/database/base.py](file://pkg/database/base.py#L84-L216)
- [internal/dao/user.py](file://internal/dao/user.py#L9-L21)
- [internal/services/user.py](file://internal/services/user.py#L38-L66)

### 密码处理功能
- 密码加密
  - 使用 bcrypt.gensalt() 生成随机盐值
  - 使用 bcrypt.hashpw() 对密码进行加密
  - 返回 UTF-8 编码的哈希字符串
- 密码验证
  - 使用 bcrypt.checkpw() 验证密码与哈希是否匹配
  - 自动处理异常情况，防止安全漏洞
- 重哈希检查
  - 使用 bcrypt.hashpw_needs_rehash() 检查是否需要重新加密
  - 支持 bcrypt 算法升级检测

**章节来源**
- [internal/utils/password.py](file://internal/utils/password.py#L16-L83)

### 使用示例（代码片段路径）
- 创建用户
  - [创建与保存示例](file://tests/orm/test_orm.py#L122-L134)
- 更新用户
  - [更新示例](file://tests/orm/test_orm.py#L143-L151)
- 查询用户
  - [按主键查询示例](file://tests/orm/test_orm_json_type.py#L124-L129)
  - [按手机号查询示例](file://internal/dao/user.py#L9-L12)
  - [按用户名查询示例](file://internal/dao/user.py#L14-L16)
- 服务层调用
  - [服务层方法调用](file://internal/services/user.py#L14-L16)
  - [密码验证示例](file://internal/services/user.py#L22-L36)
  - [用户创建示例](file://internal/services/user.py#L38-L66)
- 控制器依赖注入
  - [API 控制器](file://internal/controllers/api/user.py#L10-L16)