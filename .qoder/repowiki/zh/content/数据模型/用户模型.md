...
## 更新摘要
**所做更改**
- 更新测试用例名称从 `test_create_and_save_strictness` 到 `test_create_and_insert_strictness`
- 更新 CRUD 操作说明以反映 API 使用模式的变化
- 修正创建用户的示例代码路径引用
...
- CRUD 与软删除
  - 支持 insert（严格插入）、update（严格更新）、save（保存，新对象插入，已存在对象更新）、soft_delete（软删除）、restore（恢复）
...
## 故障排查指南
- 插入/更新异常
  - insert() 仅用于新对象插入，若对象已存在会抛出异常；update() 仅用于已存在记录更新
  - save() 会根据对象状态自动选择插入或更新，新对象插入，已存在对象更新
- 会话未初始化
...
**章节来源**
- [internal/models/user.py](file://internal/models/user.py#L31-L35)
...
**章节来源**
- [internal/models/user.py](file://internal/models/user.py#L8-L26)
- [pkg/database/base.py](file://pkg/database/base.py#L68-L74)
...
### CRUD 操作实现要点
- 创建
  - 使用 ModelMixin.create(...) 填充默认字段，随后 insert(session_provider)
  - 或使用 save(session_provider) 让框架自动判断插入或更新
- 更新
...
### 使用示例（代码片段路径）
- 创建用户
  - [创建与插入示例](file://tests/orm/test_orm.py#L120-L138)
  - [创建与插入示例（JSON类型）](file://tests/orm/test_orm_json_type.py#L117-L133)
- 更新用户
  - [更新示例](file://tests/orm/test_orm.py#L141-L160)
  - [更新示例（JSON类型）](file://tests/orm/test_orm_json_type.py#L136-L163)
- 查询用户
